// ================================================================
//  SKY FOUNDATION – ADMIN PORTAL  |  main-enhanced.js  (FIXED)
//  All code is inside ONE DOMContentLoaded listener.
//  Bugs fixed:
//    1. Duplicate loginForm submit listener removed
//    2. All code moved inside DOMContentLoaded so DOM exists
//    3. Extra closing }); at the end removed
// ================================================================
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function showDashboard(email) {
        ...
        // Build opportunity UI after dashboard is shown
    ...
    function checkStrength(val) {
        ...
        bars.forEach((id, i) => {
            const el = document.getElementById(id);
            if (!el) return;
            el.style.background = i < score ? colors[score - 1] : '';
            el.classList.toggle('active', i < score);
        });
        if (labelEl) labelEl.textContent = val ? labels[score - 1] || '' : '';
    }
    ...
    document.querySelectorAll('.nav-item[data-page]').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            const titles = {
                dashboard: 'Dashboard', learner: 'Learner Management',
                verifier: 'Verifier Management', collaborator: 'Collaborator',
                opportunity: 'Opportunity Management', reports: 'Reports and Analytics'
            };
    ...
    window.changeChartPeriod = function (period) {
        ...
        const la = document.getElementById('lineArea');
        if (lp) lp.setAttribute('d', path);
        if (la) la.setAttribute('d', path + ' L400,150 L0,150 Z');
    };
    ...
    //  COURSE MODAL
    // ============================================================
    window.openCourseDetails = function (courseName, stats) {
    ...
    if (courseModal) {
        courseModal.addEventListener('click', function (e) {
            if (e.target === this) window.closeCourseModal();
        });
    }

    // ============================================================
    //  OPPORTUNITY DETAILS MODAL (legacy)
    // ============================================================
    ...
    window.closeOpportunityDetailsModal = function () {
        document.getElementById('opportunityDetailsModal').classList.remove('active');
    };
    window.applyToOpportunity = function () {
        showToast('Application submitted successfully!');
        window.closeOpportunityDetailsModal();
    };
    const oppDetailsModal = document.getElementById('opportunityDetailsModal');
    if (oppDetailsModal) {
        oppDetailsModal.addEventListener('click', function (e) {
            if (e.target === this) window.closeOpportunityDetailsModal();
        });
    }
    ...
    // ============================================================
    window.openCollaboratorCourses = function (name, role) {
    ...
    window.closeCollaboratorCoursesModal = function () {
        document.getElementById('collaboratorCoursesModal').classList.remove('active');
    };
    window.approveCourse = function (courseName) { showToast(courseName + ' has been approved!'); };
    window.rejectCourse  = function (courseName) { showToast(courseName + ' has been rejected.'); };
    window.viewCourseDetails = function (courseName) { showToast('Viewing details for ' + courseName); };
    const collabModal = document.getElementById('collaboratorCoursesModal');
    if (collabModal) {
        collabModal.addEventListener('click', function (e) {
            if (e.target === this) window.closeCollaboratorCoursesModal();
        });
    }
    ...
    // ============================================================
    window.openQuickAddModal  = function () { document.getElementById('quickAddModal').classList.add('active'); };
    window.closeQuickAddModal = function () { document.getElementById('quickAddModal').classList.remove('active'); };
    const quickAddModal = document.getElementById('quickAddModal');
    if (quickAddModal) {
        quickAddModal.addEventListener('click', function (e) {
            if (e.target === this) window.closeQuickAddModal();
        });
    }
    ...
    // ============================================================
    window.openBulkUploadModal  = function () { document.getElementById('bulkUploadModal').classList.add('active'); };
    window.closeBulkUploadModal = function () { document.getElementById('bulkUploadModal').classList.remove('active'); };
    const bulkUploadModal = document.getElementById('bulkUploadModal');
    if (bulkUploadModal) {
        bulkUploadModal.addEventListener('click', function (e) {
            if (e.target === this) window.closeBulkUploadModal();
        });
    }
    ...
    window.handleFileSelect = function (event) {
        ...
        const fn = document.getElementById('fileName');
        if (file && fn) fn.textContent = '✓ Selected: ' + file.name;
    };
    ...
    // ============================================================
    window.openQuickAddVerifierModal  = function () { document.getElementById('quickAddVerifierModal').classList.add('active'); };
    window.closeQuickAddVerifierModal = function () { document.getElementById('quickAddVerifierModal').classList.remove('active'); };
    const quickAddVerifierModal = document.getElementById('quickAddVerifierModal');
    if (quickAddVerifierModal) {
        quickAddVerifierModal.addEventListener('click', function (e) {
            if (e.target === this) window.closeQuickAddVerifierModal();
        });
    }
    const quickAddVerifierForm = document.getElementById('quickAddVerifierForm');
    if (quickAddVerifierForm) {
        quickAddVerifierForm.addEventListener('submit', function (e) {
            e.preventDefault();
            showToast('Verifier added successfully! Email invitation sent.');
            window.closeQuickAddVerifierModal();
            this.reset();
        });
    }
    ...
    // ============================================================
    window.openBulkUploadVerifierModal  = function () { document.getElementById('bulkUploadVerifierModal').classList.add('active'); };
    window.closeBulkUploadVerifierModal = function () { document.getElementById('bulkUploadVerifierModal').classList.remove('active'); };
    const bulkUploadVerifierModal = document.getElementById('bulkUploadVerifierModal');
    if (bulkUploadVerifierModal) {
        bulkUploadVerifierModal.addEventListener('click', function (e) {
            if (e.target === this) window.closeBulkUploadVerifierModal();
        });
    }
    const bulkUploadVerifierForm = document.getElementById('bulkUploadVerifierForm');
    if (bulkUploadVerifierForm) {
        bulkUploadVerifierForm.addEventListener('submit', function (e) {
            e.preventDefault();
            ...
            window.closeBulkUploadVerifierModal();
            this.reset();
            const vfn = document.getElementById('verifierFileName');
            if (vfn) vfn.textContent = '';
        });
    }
    window.handleVerifierFileSelect = function (event) {
        const file = event.target.files[0];
        const vfn = document.getElementById('verifierFileName');
        if (file && vfn) vfn.textContent = '✓ Selected: ' + file.name;
    };
    window.downloadSampleVerifierCSV = function () {
    ...
    // ============================================================
    window.openVerifierDetails = function (name, stats) {
    ...
    const verifierDetailsModal = document.getElementById('verifierDetailsModal');
    if (verifierDetailsModal) {
        verifierDetailsModal.addEventListener('click', function (e) {
            if (e.target === this) window.closeVerifierDetailsModal();
        });
    }
    ...
    //  VERIFIER FILTERS
    // ============================================================
    window.filterVerifiers = function () {
    ...
    // ============================================================
    //  LOGIN FORM  (single listener — the only one)
    // ============================================================
    ...
    if (signupForm) {
        signupForm.addEventListener('submit', function (e) {
            ...
            if (!captchaInput) {
                ...
                showError('signupCaptchaErr', 'Captcha does not match.'); generateCaptcha('signup'); valid = false;
            }
    ...
    if (forgotForm) {
        forgotForm.addEventListener('submit', function (e) {
            ...
            if (!captchaInput) {
                ...
                showError('forgotCaptchaErr', 'Captcha does not match.'); generateCaptcha('forgot'); valid = false;
            }
    ...
    function injectOpportunityStyles() {
        ...
        style.textContent = `
            .opp-toolbar { display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:22px; }
            .opp-search-wrap { position:relative;flex:1;min-width:180px; }
            .opp-search-wrap svg { position:absolute;left:12px;top:50%;transform:translateY(-50%);width:16px;height:16px;stroke:#888;fill:none;stroke-width:2;stroke-linecap:round;pointer-events:none; }
            .opp-search-input { width:100%;padding:9px 12px 9px 38px;border:1.5px solid var(--border,#e0e0e0);border-radius:10px;font-size:13.5px;background:var(--card-bg,#fff);color:var(--text,#222);outline:none;transition:border-color .2s;box-sizing:border-box; }
            .opp-search-input:focus { border-color:var(--primary,#4f46e5); }
            .opp-filter-select,.opp-sort-select { padding:9px 14px;border:1.5px solid var(--border,#e0e0e0);border-radius:10px;font-size:13px;background:var(--card-bg,#fff);color:var(--text,#222);cursor:pointer;outline:none; }
            .opp-add-btn { display:flex;align-items:center;gap:7px;padding:9px 18px;background:var(--primary,#4f46e5);color:#fff;border:none;border-radius:10px;font-size:13.5px;font-weight:600;cursor:pointer;transition:background .2s,transform .15s;white-space:nowrap; }
            ...
            .opp-results-bar { display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;font-size:13px;color:var(--text-muted,#666); }
            ...
            .opp-card { background:var(--card-bg,#fff);border:1.5px solid var(--border,#e8e8e8);border-radius:16px;padding:20px 22px 18px;display:flex;flex-direction:column;gap:14px;transition:box-shadow .25s,transform .2s;animation:oppCardIn .35s ease both;position:relative;overflow:hidden; }
            .opp-card::before { content:'';position:absolute;top:0;left:0;right:0;height:3px;background:var(--primary,#4f46e5);border-radius:16px 16px 0 0;opacity:0;transition:opacity .2s; }
            .opp-card:hover { box-shadow:0 8px 30px rgba(0,0,0,.10);transform:translateY(-3px); }
            .opp-card:hover::before { opacity:1; }
            @keyframes oppCardIn { from{opacity:0;transform:translateY(18px)} to{opacity:1;transform:translateY(0)} }
            .opp-card-header { display:flex;align-items:flex-start;gap:12px; }
            .opp-icon { width:44px;height:44px;border-radius:12px;background:var(--primary-light,#ede9fe);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:20px; }
            .opp-card-title-wrap { flex:1;min-width:0; }
            .opp-card-title { font-size:15px;font-weight:700;color:var(--text,#1a1a2e);margin:0 0 4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis; }
            .opp-category-badge { display:inline-block;padding:2px 10px;border-radius:20px;font-size:11px;font-weight:600;background:var(--primary-light,#ede9fe);color:var(--primary,#4f46e5);letter-spacing:.3px;text-transform:uppercase; }
            .opp-stats { display:grid;grid-template-columns:repeat(3,1fr);gap:8px;background:var(--bg-soft,#f7f7fb);border-radius:10px;padding:10px 12px; }
            ...
            .opp-skill-tag { padding:3px 10px;border-radius:20px;background:var(--tag-bg,#f0f0f5);color:var(--text-muted,#555);font-size:11px;font-weight:500; }
            .opp-skill-more { padding:3px 10px;border-radius:20px;border:1px solid var(--border,#ddd);color:var(--text-muted,#888);font-size:11px;background:transparent; }
            .opp-card-actions { display:flex;gap:7px;margin-top:auto;padding-top:4px;border-top:1px solid var(--border,#f0f0f0); }
            .opp-btn { flex:1;padding:7px 0;border-radius:8px;border:none;font-size:12.5px;font-weight:600;cursor:pointer;transition:background .18s,transform .12s;display:flex;align-items:center;justify-content:center;gap:5px; }
            ...
            .opp-skeleton { background:var(--card-bg,#fff);border:1.5px solid var(--border,#e8e8e8);border-radius:16px;padding:20px 22px;display:flex;flex-direction:column;gap:14px; }
            ...
            .opp-modal-overlay { position:fixed;inset:0;background:rgba(0,0,0,.45);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;z-index:9999;opacity:0;pointer-events:none;transition:opacity .25s; }
            .opp-modal-overlay.open { opacity:1;pointer-events:all; }
            .opp-modal { background:var(--card-bg,#fff);border-radius:20px;width:min(540px,94vw);max-height:88vh;overflow-y:auto;box-shadow:0 24px 64px rgba(0,0,0,.2);transform:scale(.93) translateY(24px);transition:transform .28s cubic-bezier(.34,1.56,.64,1); }
            .opp-modal-overlay.open .opp-modal { transform:scale(1) translateY(0); }
            .opp-modal-header { display:flex;align-items:flex-start;justify-content:space-between;padding:24px 26px 18px;border-bottom:1px solid var(--border,#f0f0f0);position:sticky;top:0;background:var(--card-bg,#fff);border-radius:20px 20px 0 0;z-index:1; }
            .opp-modal-title { font-size:18px;font-weight:800;color:var(--text,#111);margin:0; }
            .opp-modal-close { background:var(--bg-soft,#f5f5fa);border:none;border-radius:50%;width:32px;height:32px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--text,#555);font-size:18px;transition:background .15s;flex-shrink:0; }
            ...
            .opp-detail-skill-tag { padding:4px 12px;border-radius:20px;background:var(--primary-light,#ede9fe);color:var(--primary,#4f46e5);font-size:12px;font-weight:600; }
            .opp-modal-footer { padding:16px 26px 24px;display:flex;justify-content:flex-end;gap:10px; }
            .opp-modal-footer-btn { padding:9px 22px;border-radius:10px;border:none;font-size:13.5px;font-weight:600;cursor:pointer;transition:background .18s,transform .12s; }
            ...
            .opp-form-group input,.opp-form-group select,.opp-form-group textarea { padding:9px 12px;border:1.5px solid var(--border,#e0e0e0);border-radius:9px;font-size:13.5px;background:var(--card-bg,#fff);color:var(--text,#222);outline:none;transition:border-color .2s;resize:vertical; }
            .opp-form-group input:focus,.opp-form-group select:focus,.opp-form-group textarea:focus { border-color:var(--primary,#4f46e5); }
            .opp-confirm-overlay { position:fixed;inset:0;background:rgba(0,0,0,.5);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;z-index:10000;opacity:0;pointer-events:none;transition:opacity .2s; }
            .opp-confirm-overlay.open { opacity:1;pointer-events:all; }
            .opp-confirm-box { background:var(--card-bg,#fff);border-radius:18px;padding:30px 28px 24px;width:min(380px,92vw);text-align:center;box-shadow:0 20px 60px rgba(0,0,0,.18);transform:scale(.9);transition:transform .22s cubic-bezier(.34,1.56,.64,1); }
    ...
    function buildOpportunityUI() {
        ...
        // Preserve the existing header + Add button if present
        section.innerHTML = `
    ...
    function loadOpportunities() {
        fetch('/opportunities', { method: 'GET', credentials: 'include' })
            ...
            .catch(() => renderOpportunities([]));
    }

    window.filterAndRenderOpportunities = function () {
    ...
    // VIEW MODAL
    window.openOppViewModal = function (id) {
    ...
    window.closeOppViewModal = function () {
        const el = document.getElementById('oppViewOverlay');
        if (el) el.classList.remove('open');
    };
    window.openEditFromView = function () {
        window.closeOppViewModal();
        if (currentViewOp) window.openOppEditModal(currentViewOp.id);
    };

    // EDIT MODAL
    window.openOppEditModal = function (id) {
        const op = allOpportunities.find(o => o.id === id);
    ...
    window.closeOppEditModal = function () {
        const el = document.getElementById('oppEditOverlay');
        if (el) el.classList.remove('open');
        currentEditId = null;
    };
    window.submitEditOpportunity = function () {
        ...
        .then(() => {
            ...
            window.closeOppEditModal();
            window.filterAndRenderOpportunities();
        })
    ...
    window.promptDeleteOpportunity = function (id, name) {
        _pendingDeleteId = id;
    ...
    window.closeOppConfirm = function () {
        const el = document.getElementById('oppConfirmOverlay');
        if (el) el.classList.remove('open');
        _pendingDeleteId = null;
    };
    window.confirmDeleteOpportunity = function () {
        if (!_pendingDeleteId) return;
        const id = _pendingDeleteId;
        window.closeOppConfirm();
    ...
    window.closeOpportunityModal = function () { document.getElementById('opportunityModal').classList.remove('active'); };

    const opportunityModal = document.getElementById('opportunityModal');
    if (opportunityModal) {
        opportunityModal.addEventListener('click', function (e) {
            if (e.target === this) window.closeOpportunityModal();
        });
    }
    ...
    window.handleOppOverlayClick = function (e, overlayId) {
        if (e.target.id === overlayId) {
            if (overlayId === 'oppViewOverlay') window.closeOppViewModal();
            if (overlayId === 'oppEditOverlay') window.closeOppEditModal();
        }
    };
    ...
    document.addEventListener('keydown', function (e) {
        ...
        window.closeOpportunityModal();
        window.closeOpportunityDetailsModal();
        window.closeCollaboratorCoursesModal();
        window.closeQuickAddModal();
        window.closeBulkUploadModal();
        window.closeQuickAddVerifierModal();
        window.closeBulkUploadVerifierModal();
        window.closeVerifierDetailsModal();
        window.closeOppViewModal();
        window.closeOppEditModal();
        window.closeOppConfirm();
    });
...
}); // END DOMContentLoaded — only one closing bracket here
