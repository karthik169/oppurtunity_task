        ...
        data.forEach(op => {
            ...
            card.innerHTML = `
                <div class="course-header">
                    <div>
                        <h5>${op.title}</h5>
                        <div class="course-category">${op.category}</div>
                    </div>
                </div>
                ...
                <div class="course-stats">
                    <div class="course-stat">
                        ...
                        <div class="course-stat-value">${op.duration}</div>
                    </div>

                    <div class="course-stat">
                        <div class="course-stat-label">Start Date</div>
                        <div class="course-stat-value">${op.start_date}</div>
                    </div>

                    <div class="course-stat">
                        <div class="course-stat-label">Applicants</div>
                        <div class="course-stat-value">${op.max_applicants || 0}</div>
                    </div>
                </div>
            ...
            card.querySelector('.view-btn').addEventListener('click', () => {
                document.getElementById('d_title').innerText = op.title;
                document.getElementById('d_category').innerText = op.category;
                document.getElementById('d_duration').innerText = op.duration;
                document.getElementById('d_start').innerText = op.start_date;
                document.getElementById('d_desc').innerText = op.description;
                document.getElementById('d_skills').innerText = op.skills;
                document.getElementById('d_future').innerText = op.future_opportunities;
                document.getElementById('d_max').innerText = op.max_applicants;

                document.getElementById('detailsModal').style.display = 'block';
            });
            // ✏️ EDIT BUTTON
            card.querySelector('.edit-btn').addEventListener('click', () => {
                alert("Edit feature coming soon for: " + op.title);

                // 👉 later we will open form + prefill data
            });
